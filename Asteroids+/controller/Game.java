...
public class Game implements Runnable, KeyListener {
	...
	// for possible future use
	 ...
	 PUSH = 65, 				// a key arrow
	// NUM_ENTER = 10, 				// hyp
	 ODIN= 70; 					// fire special weapon;  F key

	private Clip clpThrust;
	private Clip clpMusicBackground;

	private static final int SPAWN_NEW_ENEMY = 1200;
	...
	public Game() {
        ...
        clpMusicBackground = Sound.clipForLoopFactory("space.wav");
		...
		for (Movable movFriend : CommandCenter.movFriends) {
			for (Movable movFoe : CommandCenter.movFoes) {
				...
				if (pntFriendCenter.distance(pntFoeCenter) < (nFriendRadiux + nFoeRadiux)) {
                    ...
                    else if ((movFriend instanceof ForcePush)){
                        pushFoe((Sprite)movFriend, (Sprite)movFoe);
                    }
					//not the falcon
					else {
						tupMarkForRemovals.add(new Tuple(CommandCenter.movFriends, movFriend));
		...
		if (CommandCenter.getFalcon() != null){
			...
			for (Movable movFloater : CommandCenter.movFloaters) {
				...
				if (pntFalCenter.distance(pntFloaterCenter) < (nFalRadiux + nFloaterRadiux)){
                    ...
                    } else if(movFloater instanceof NewOdinFloater){
                        CommandCenter.setOdinSat(CommandCenter.getOdinSat()+1);
                    } else if(movFloater instanceof NewForcePushFloater){
                        CommandCenter.setForcePush(CommandCenter.getForcePush()+1);
                    } else if(movFloater instanceof NewSlowFloater){
                        CommandCenter.setSlow(CommandCenter.getSlow()+1);
                    }
    ...
    public void pushFoe(Sprite movFriend, Sprite movFoe){
        ArrayList<Double> deltas = movFriend.getDeltas(movFoe);
        double oldDeltaX = movFoe.getDeltaX();
        double oldDeltaY = movFoe.getDeltaY();
        double oldVelocity = Math.sqrt(Math.pow(oldDeltaX, 2) + Math.pow(oldDeltaY, 2));

        movFoe.setDeltaX(-1*deltas.get(0)*oldVelocity);
        movFoe.setDeltaY(-1*deltas.get(1)*oldVelocity);
    ...
    private void spawnEnemy(){
        if (nTick % (SPAWN_NEW_ENEMY - nLevel * 8) == 0) {
            CommandCenter.movFoes.add(new Enemy());
            Sound.playSound("enemy.wav");
        }
    }

	private void spawnNewFloater() {
		//make the appearance of power-up dependent upon ticks and levels
		//the higher the level the more frequent the appearance
        int rand = (int)(Math.random() * 4) + 1;
		if (nTick % (SPAWN_NEW_SHIP_FLOATER - nLevel * 4) == 0) {
			if(rand == 1){
                CommandCenter.movFloaters.add(new NewShipFloater());
            } else if(rand == 2){
                CommandCenter.movFloaters.add(new NewOdinFloater());
            } else if(rand == 3){
                CommandCenter.movFloaters.add(new NewForcePushFloater());
            } else {
                CommandCenter.movFloaters.add(new NewSlowFloater());
            }
		}
	}
	...
	private void startGame() {
        ...
        CommandCenter.setOdinSat(5);
        CommandCenter.setForcePush(5);
        CommandCenter.setSlow(5);
		if(!bMuted){
	...
	public void keyPressed(KeyEvent e) {
		...
		if (fal != null) {
			...
			// case KILL:
			case PUSH:
                if(CommandCenter.getForcePush() > 0){
                    CommandCenter.movFriends.add(new ForcePush());
                    CommandCenter.setForcePush(CommandCenter.getForcePush()-1);
                    Sound.playSound("shieldup.wav");
                }
                break;
			case SLOW:
                if(CommandCenter.getSlow() > 0){
                    CommandCenter.movFriends.add(new Slow());
                    CommandCenter.setSlow(CommandCenter.getSlow()-1);
                    Sound.playSound("slow.wav");
                }
                break;
			default:
				break;
	...
	public void keyReleased(KeyEvent e) {
		...
		if (fal != null) {
			...
			//ODIN is an "satellite" that launches from the falcon and fires continuously until it expires
			case ODIN:
				if (CommandCenter.getOdinSat() > 0){
                CommandCenter.movFriends.add(new Odin(fal));
                    CommandCenter.setOdinSat(CommandCenter.getOdinSat() - 1);
                }
				Sound.playSound("odin.wav");
				break;
