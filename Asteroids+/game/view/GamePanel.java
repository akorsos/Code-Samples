...
import edu.uchicago.cs.java.finalproject.game.model.Movable;
 ...
 public class GamePanel extends Panel {
	...
	public GamePanel(Dimension dim){
		...
		gmf.setSize(dim);
		gmf.setTitle("Mega Super Ultimate Asteroids");
		gmf.setResizable(false);
		gmf.setVisible(true);
		this.setFocusable(true);

        for(int i = 0; i < 100; i++){
            int starH = (int)(Math.random()*Game.DIM.height);
            int starW = (int)(Math.random()*Game.DIM.width);
            Point s = new Point(starW,starH);
            stars.add(s);
        }
	}
	...
	private void drawScore(Graphics g) {
        ...
        g.drawString("Level: " + CommandCenter.getLevel(), nFontWidth, nFontHeight+15);
        g.drawString("Odin Satellite: " + CommandCenter.getOdinSat(), nFontWidth, nFontHeight+30);
        g.drawString("Force Push: " + CommandCenter.getForcePush(), nFontWidth, nFontHeight+45);
        g.drawString("Slow Time: " + CommandCenter.getSlow(), nFontWidth, nFontHeight+60);
	}
	...
	public void update(Graphics g) {
        ...
        for(int i = 0; i < stars.size(); i++){
            grpOff.fillRect(stars.get(i).x, stars.get(i).y, 3, 3);
        }
	...
	private void displayTextOnScreen() {
		...
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 200);
		strDisplay = "'A' for Force Push";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 240);

		strDisplay = "'F' for Odin Satellite";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 280);

		strDisplay = "'D' for Slow Time";
		grpOff.drawString(strDisplay,
				(Game.DIM.width - fmt.stringWidth(strDisplay)) / 2, Game.DIM.height / 4
						+ nFontHeight + 320);
	}
