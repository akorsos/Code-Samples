...
public abstract class Sprite implements Movable {
    ...
    public ArrayList<Double> getDeltas(Movable mov2){
        Point mov1Pnt = this.getCenter();
        Point mov2Pnt = mov2.getCenter();
        double diffX = mov2Pnt.x - mov1Pnt.x;
        double diffY = mov2Pnt.y - mov1Pnt.y;
        double diffH = Math.sqrt(Math.pow(diffX, 2) + Math.pow(diffY, 2));
        ArrayList<Double> ret = new ArrayList<Double>();
        ret.add(-1*diffX/diffH);
        ret.add(-1*diffY/diffH);
        return ret;
    }
